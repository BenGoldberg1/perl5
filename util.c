...
Perl_mess_sv(pTHX_ SV *basemsg, bool consume)
{
...
#if defined(USE_C_BACKTRACE) && defined(USE_C_BACKTRACE_ON_WARN)
    {
        char *ws, *ds;
        int di;
        /* The PERL_C_BACKTRACE_ON_WARN must be an integer of one
         * (or more), and the PERL_C_BACKTRACE_DEPTH must be
         * an integer of one or more. */
        if ((ws = PerlEnv_getenv("PERL_C_BACKTRACE_ON_WARN")) &&
            atoi(ws) > 0 &&
            (ds = PerlEnv_getenv("PERL_C_BACKTRACE_DEPTH")) &&
            (di = atoi(ds)) > 0) {
            Perl_dump_c_backtrace(aTHX_ Perl_debug_log, di, 1);
        }
    }
#endif
