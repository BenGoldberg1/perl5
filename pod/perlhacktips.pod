...
automatic dumping of the backtrace just before a warnings is emitted
(this includes coincidentally croaking) by adding
...
you need to enable it in runtime with B<two> environment variables:
C<PERL_C_BACKTRACE_ON_WARN=1 PERL_C_BACKTRACE_DEPTH=10>.  The first
one enables the dump, the second one tells how much to dump.  B<NOTE:
this is far from an ideal interface, and intentionally somewhat hard to enable>--
better API/UI suggestions are welcome.
